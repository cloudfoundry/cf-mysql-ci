...
export BOSH_ALL_PROXY="ssh+socks5://jumpbox@\${BOSH_ENVIRONMENT}:22?private-key=\${BOSH_GW_PRIVATE_KEY}"

credhub_api_url="https://\$(cat \${bosh_lite_info_dir}/external-ip | tr '\\n' '\\0'):8844/api/"
...
if \$(bosh -d cf deployment > /dev/null 2>&1); then
  ...
  cf_admin_password="\$(credhub get -n /lite/cf/cf_admin_password -j | jq .value -r)"
